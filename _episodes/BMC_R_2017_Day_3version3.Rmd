...
![Measures of Central Tendancy](central_tendency.gif)
...
You get most of the same summary statistics visually from a boxplot:
```{r}
boxplot(x,
        horizontal = T)
```
...
##  Hypothesis testing 
A hypothesis test will tell you the probability that the data you have is generated by the model (your hypothesis).
For example, I may have rolled two dice



A hypothesis test can determine whether an associative relationship exists, e.g.: is the weight of response to unit change in input larger than zero?
